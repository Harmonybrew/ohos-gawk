...
本项目为 OpenHarmony 平台编译了 gawk，并发布预构建包。

本项目是 Harmonybrew 项目基础设施的一部分，主要供内部使用。我们也将其开源，有需要者可自取。

## 获取预构建包

前往 [release 页面](https://github.com/Harmonybrew/ohos-gawk/releases) 获取。
...
```sh
cd ~
curl -fLO https://github.com/Harmonybrew/ohos-gawk/releases/download/5.3.2/gawk-5.3.2-ohos-arm64.tar.gz
...
示例：
```sh
hdc file send gawk-5.3.2-ohos-arm64.tar.gz /data
hdc shell
...
需要用一台 Linux x64 服务器来运行项目里的 build.sh，以实现 gawk 的交叉编译。

这里以 Ubuntu 24.04 x64 作为示例：
```sh
sudo apt update && sudo apt install -y build-essential unzip
git clone https://github.com/Harmonybrew/ohos-gawk.git
cd ohos-gawk
./build.sh
```
